...
#----------------------------------------------------------------------------
# VE-SDE diffuser class for diffusion in 3D rotational (SO(3)) space 
...
class SO3Diffuser:
    ...
    def torch_score(
        ...
        return omega_scores_t * vec / (omega[..., None] + eps)
    ...
    def forward_marginal(self, t: float, rot_0: np.ndarray = np.zeros((1, 3))):
        ...
        """
        sampled_rots = self.sample(t, n_samples=1)
