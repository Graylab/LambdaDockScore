...
class Sampler:
    def __init__(
        ...
        if self.data_conf.use_finetuned_ranking_model:
            self.ranking_model = Score_Model.load_from_checkpoint(
                self.data_conf.finetune_ranking_model_ckpt,
                map_location=self.device,
            )
        ...
        for batch in tqdm(self.test_dataloader):
            ...
            if self.data_conf.get_gt_energy:
                ...
                for i in range(self.data_conf.num_samples):
                    _rec_pos, _lig_pos, ranking_energy, ranking_num_clashes, baseline_energy, baseline_num_clashes = self.Euler_Maruyama_sampler(
                        batch=batch,
                        batch_size=1,
                        eps=1e-3,
                    ...
                    metrics.update({'ranking_energy': ranking_energy.item()})
                    metrics.update({'ranking_num_clashes': ranking_num_clashes.item()})
                    metrics.update({'baseline_energy': baseline_energy.item()})
                    metrics.update({'baseline_num_clashes': baseline_num_clashes.item()})
                    metrics_list.append(metrics)
            ...
            for i, time_step in enumerate(tqdm((time_steps))):  
                ...
                if is_last:
                    ...
                    if self.data_conf.use_finetuned_ranking_model:
                        ...
                        baseline_output = self.model(batch)
                    else:
                        ranking_output = self.model(batch)
                        baseline_output = ranking_output

                # save coordinates
                rec_trj.append(rec_pos)         
                lig_trj.append(lig_pos)

        return rec_trj, lig_trj, ranking_output["energy"], ranking_output["num_clashes"], baseline_output["energy"], baseline_output["num_clashes"]
...
def main(config: DictConfig):
    ...
    base_filename = config.data.out_csv
    ranking_filename = os.path.join(output_dir, base_filename.replace(".csv", "_ranking.csv"))
    baseline_filename = os.path.join(output_dir, base_filename.replace(".csv", "_baseline.csv"))

    with open(ranking_filename, "w", newline="") as ranking_csvfile, open(baseline_filename, "w", newline="") as baseline_csvfile:
        results = sampler.run_sampling()

        # Split results into ranking and baseline lists
        ranking_results = [
            {k: v for k, v in row.items() if k not in ["baseline_energy", "baseline_num_clashes"]}
            for row in results
        ]
        baseline_results = [
            {k: v for k, v in row.items() if k not in ["ranking_energy", "ranking_num_clashes"]}
            for row in results
        ]

        # Write ranking results to CSV
        if ranking_results:
            ranking_header = list(ranking_results[0].keys())
            ranking_writer = csv.DictWriter(ranking_csvfile, fieldnames=ranking_header)
            ranking_writer.writeheader()
            for row in ranking_results:
                ranking_writer.writerow(row)

        # Write baseline results to CSV
        if baseline_results:
            baseline_header = list(baseline_results[0].keys())
            baseline_writer = csv.DictWriter(baseline_csvfile, fieldnames=baseline_header)
