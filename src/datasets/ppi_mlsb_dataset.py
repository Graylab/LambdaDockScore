...
#----------------------------------------------------------------------------
#Constants (sorry for the hardcoding)
FULL_EMBEDDING_DIM = 1301  # This is the full embedding dimension used in the model
ESM_EMBEDDING_DIM = 1280  # This is the ESM embedding dimension used in the model
ONE_HOT_EMBEDDING_DIM = 21  # This is the one-hot embedding dimension used in the model
...
class PPIDataset(Dataset):
    def __init__(
        ...
        # Get project root (assuming this file is in src/datasets/)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../"))
        data_root = os.path.join(project_root, "data")

        if dataset == 'dips_train_0.3_rep':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/train_0.3_rep.txt")

        elif dataset == 'dips_val_0.3_rep':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/val_0.3_rep.txt")

        elif dataset == 'dips_train':
            self.data_dir = os.path.join(data_root, "pt/dips_clean")
            self.data_list = os.path.join(data_root, "splits/train.txt")

        elif dataset == 'dips_val':
            self.data_dir = os.path.join(data_root, "pt/dips_clean")
            self.data_list = os.path.join(data_root, "splits/val.txt")

        elif dataset == 'dips_train_hetero':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/dips_train_hetero.txt")

        elif dataset == 'dips_val_hetero':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/dips_val_hetero.txt")

        elif dataset == 'dips_test':
            self.data_dir = os.path.join(data_root, "pt/dips_test")
            self.data_list = os.path.join(data_root, "splits/dips_test.txt")

        elif dataset == 'db5_test':
            self.data_dir = os.path.join(data_root, "pt/db5_bound")
            self.data_list = os.path.join(data_root, "splits/db5_test.txt")
            
        elif dataset == 'db5_bound':
            self.data_dir = os.path.join(data_root, "pt/db5_bound")
            self.data_list = os.path.join(data_root, "splits/db5_bound.txt")

        elif dataset == 'db5_all':
            self.data_dir = os.path.join(data_root, "pt/db5_bound")
            self.data_list = os.path.join(data_root, "splits/db5_all.txt")
        
        elif dataset == 'db5_debug':
            self.data_dir = os.path.join(data_root, "pt/db5_bound")
            self.data_list = os.path.join(data_root, "splits/db5_debug.txt")

        elif dataset == 'db5_ab_ag':
            self.data_dir = os.path.join(data_root, "pt/db5_bound")
            self.data_list = os.path.join(data_root, "splits/db5_ab_ag.txt")

        elif dataset == 'ab_ag':
            self.data_dir = os.path.join(data_root, "pt/ab_ag")
            self.data_list = os.path.join(data_root, "splits/non_db5_ab_ag.txt")

        elif dataset == 'dips_train_hetero_debug':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/dips_train_hetero_debug.txt")
        
        elif dataset == 'dips_val_hetero_debug':
            self.data_dir = os.path.join(data_root, "pt/dips_bb")
            self.data_list = os.path.join(data_root, "splits/dips_val_hetero_debug.txt")
            ...
            print(f"Warning: Dataset {dataset} not explicitly handled with local paths. Using default or empty.")
            self.data_dir = None
            self.data_list = None
            self.file_list = []
        ...
        if self.data_list and os.path.exists(self.data_list):
            ...
            self.file_list = [line.strip() for line in lines]
             ...
             print(f"Warning: Data list file not found: {self.data_list}")
             self.file_list = []
    ...
    def __getitem__(self, idx: int):
        ...
        if self.dataset[:4] == 'dips':
            ...
            _id = self.file_list[idx]
        file_path = os.path.join(self.data_dir, _id + '.pt')
        ...
        if self.training:
            ...
            swap = False
        return self.construct_full_output(raw_data=data, _id=_id, swap=swap, file_path=file_path)

    def construct_full_output(self, raw_data, _id, swap=False, file_path=None):

        rec_x = raw_data['receptor'].x.float()
        rec_seq = raw_data['receptor'].seq
        rec_pos = raw_data['receptor'].pos.float()
        lig_x = raw_data['ligand'].x.float()
        lig_seq = raw_data['ligand'].seq
        lig_pos = raw_data['ligand'].pos.float()
        ...
        if self.use_esm:
            ...
            # Edge case: some protein poses are loaded in with one-hot embeddings already concatenated to ESM embeddings, so we only concatenate if the one-hot embeddings are not already present
            if rec_x.size(-1) == ESM_EMBEDDING_DIM:
                rec_x = torch.cat([rec_x, rec_onehot], dim=-1)
            elif rec_x.size(-1) == FULL_EMBEDDING_DIM:
                pass
            else:
                warnings.warn(f"Unexpected receptor embedding size: {rec_x.size(-1)}. Expected {FULL_EMBEDDING_DIM} or {ESM_EMBEDDING_DIM}.")

            if lig_x.size(-1) == ESM_EMBEDDING_DIM:
                lig_x = torch.cat([lig_x, lig_onehot], dim=-1)
            elif lig_x.size(-1) == FULL_EMBEDDING_DIM:
                pass
            else:
                warnings.warn(f"Unexpected ligand embedding size: {lig_x.size(-1)}. Expected {FULL_EMBEDDING_DIM} or {ESM_EMBEDDING_DIM}.")
        else:
            rec_x = rec_onehot
            lig_x = lig_onehot
        ...
        if self.training:
            ...
            if swap:
                vars_list[0], vars_list[1] = vars_list[1], vars_list[0]
