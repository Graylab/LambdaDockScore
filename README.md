...
The DB5.5 evaluation computes a pose-level bootstrap by resampling each complex's poses with replacement over 10,000 replicates, re-ranking them, and calculating the mean success rate. Because each bootstrap resample contains approximately 63% of the distinct poses, the bootstrap mean is lower than the standard point estimate when successful poses are rare. For example, the DB5.5 oracle rate is 36.0% as a point estimate but 31.1% under the pose-level bootstrap reported in the paper.
